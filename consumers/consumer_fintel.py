...
def init_db(db_path: pathlib.Path) -> None:
    ...
    try:
        ...
        con.execute("DROP SEQUENCE IF EXISTS streamed_messages_seq;")
        con.execute("CREATE SEQUENCE streamed_messages_seq START 1;")
        con.execute("""
        CREATE TABLE streamed_messages (
            id BIGINT PRIMARY KEY DEFAULT nextval('streamed_messages_seq'),
            message TEXT,
            author TEXT,
            timestamp TIMESTAMP,
            category TEXT,
...
def insert_message(message: dict, db_path: pathlib.Path) -> None:
    ...
    try:
        ...
        (message, author, timestamp, category, sentiment, keyword_mentioned, message_length, department_ID, job_classification)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);
        """,
        [
        message["message"],
